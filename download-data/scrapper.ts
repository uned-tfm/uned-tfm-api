...
import { Db, MongoClient, ObjectId, OptionalUnlessRequiredId, ServerApiVersion } from 'mongodb';
...
type MalwareData = {
  ...
  tags: string[];
  intelligence: { mail: null; clamav: string[]; downloads: string; uploads: string };
  yara_rules: unknown[];
  gimphash?: unknown;
  sha1_hash: string;
...
class MalwareScrapper {
  ...
  async processMalwareResult(result: MalwareApiResponse, force = false): Promise<void> {
    const OK_RESULT = 'ok';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const md5 of md5_ids) {
      console.log(`<-- Md5 ${counter} of ${total} [${(counter * 100) / total}%] -->`);
